...
実務の 115 機能を観察すると、テストの振る舞いは以下の 4 パターンに収束することがわかりました。
...
たとえばパターン D（条件入力）では、フォームへの入力順序を「フィールドの ID」ではなく「**画面上の視覚的な位置（上→下、左→右）**」を基準に決定します。
...
DOM 座標に依存する場合:
  画面上の入力可能要素を y 座標 → x 座標でソートし、
  CSV の searchConditions[0], [1], [2] と位置で対応させる
  ↑ ID が変わっても、視覚的な配置が変わらない限り動作する
```

この設計により、CSS リファクタリングや ID 命名規則の変更に対してテストコードが耐性を持ちます。
